...
public class Rubrica {
	...
	public Rubrica(Rubrica r) {
		...
		for(int i=0; i<contattiMemorizzati; i++)
			contatti[i] = new Contatto(r.contatti[i]);// costruttore per copia della classe Contatto
	...
	public Contatto[] estraiContatti(String nome, String cognome) {
		...
		int pos=0; // prima posizione libera di ris
		for(int i=0; i<contattiMemorizzati; i++)
			if(contatti[i].getNome().equals(nome) && contatti[i].getCognome().equals(cognome)) {
	...
	public boolean equals(Object o) {
		...
		if(!(o instanceof Rubrica)) return false;
		Rubrica r = (Rubrica) o;
		if(contattiMemorizzati!=r.contattiMemorizzati) return false;
		for(int i=0; i<contattiMemorizzati; i++)
			if(!contatti[i].equals(r.contatti[i])) return false;
		return true;
	}// equals
