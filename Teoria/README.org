...
** COMMENT Caso di studio: Implementazione di una agendina
Un'agendina è un altro esempio di tabella, ossia un elenco di nominativi. I nominativi si suppongono mantenuti
in ordine alfabetico (prima per cognome e a parità di cognome per nome). Per semplicità si ignorano le
omonimie: si suppone che non esistano due nominativi con lo stesso nome e cognome e telefono diversi.
Come si vede, l’interfaccia Comparable che presiede alla definizione del confronto naturale degli oggetti, essa
stessa è generica. Pertanto, ad una «classe è data la possibilità di introdurre il confronto utilizzando
Comparable non di Object ma del tipo specifico della classe. Si guadagna in sicurezza e semplicità. Esempio: Si vuole scrivere un intero programma che consenta di:
public class Data implements Comparable<Data>{
public int compareTo( Data d ){//si evita una operazione di casting
if( this.equals(d) ) return 0;
if( this.A<d.A II this.A==d.A && this.M<d.M II this.A==d.A && this.M==d.M && this.G<d.G )
return -1;
return +1;
}//compareTo
}//Data
Tabelle e loro rappresentazione
Una tabella è una collezione di informazioni strutturate. Un esempio classico è il registro anagrafe di un
comune. Per ogni persona, il registro memorizza: cognome, nome, data di nascita, sesso, stato civile etc.
Una tabella è logicamente rappresentabile come una griglia righe-colonne. Una versione ridotta del registro
anagrafe è la seguente:
Cognome
persona
Nome
DataNascita Sesso StatoCivile
Bianchi Fabio 10/11
/1945 M Sposato
Rossi Mario 12/03
/1985 M Celibe
Le colonne sono associate alle varie informazioni componenti. Le righe individuano le entità, in questo caso
persone. Prendendo un'intera colonna si possono osservare, ad es., tutti i possibili cognomi esistenti nella
tabella. Prendendo un'intera riga s'identifica una registrazione (da cui il termine registro) e quindi
semanticamente una persona. i S
Si dice chiave di una tabella un sottinsieme delle colonne i cui valori identificano univocamente un'entità. Ad
esempio, nel registro anagrafe di un piccolo comune, una persona potrebbe essere univocamente determinata
dal cognome, nome e data di nascita
150
•
•
•
•
•
Inserire un nuovo nominativo nella tabella
Eliminare un nominativo dalla tabella
Cercare il numero di telefono (prefisso+telefono) di una persona
Cercare il nominativo di un assegnato numero di telefono
Salvare/ripristinare l'agendina su/da file
Per raggiungere l'obiettivo si appronta prima una classe, es.Nominativo, che descrive un generico nominativo
ossia una riga della tabella. La classe Nominativo, inserita nel package poo.agendina, genera oggetti
immutabili.
...
- Nel package java.util sono presenti alcune interfacce e classi
  “pronte per l'uso” (framework) che consentono di lavorare su
  collezioni generiche di elementi
...
- Le classi concrete con bordo sottile sono classi di utilità e
  contengono metodi statici (e.g., Collections e Arrays)

** Framework
#+begin_export latex
\begin{figure}
\includegraphics[scale=0.26]{\figures/collframework.png}
\end{figure}
#+end_export
...
- Una lista è una collezione nella quale:
  ...
  + gli elementi di una lista possono essere rintracciati in base al loro indice
...
- Per fare uso delle collezioni è importante conoscere le interfacce
  Collection, List, Set, Iterator, Listlterator,Map etc
...
:BEAMER_col: 0.6
:END:
- I metodi add(), addAII(), remove(), removeAII() ritornano un boolean
  che vale true se la collezione risulta modificata a seguito
  dell'operazione
- retainAll() toglie tutti gli elementi ad eccezioni di quelli passati
  in input
...
** Esempio
#+begin_src  java
List<lnteger> li=new ArrayList<lnteger>();
...
- Utile per “navigare" sulla collection elemento per elemento
  ...
  -  hasNext() ritorna false, solleva un'eccezione
    NoSuchElementException (erede di RuntimeException).

#+begin_export latex
\begin{figure}
\includegraphics[scale=0.26]{\figures/iterator.png}
\end{figure}
#+end_export
...
- Un’invocazione it.remove() non preceduta da una chiamata it.next() solleva un’eccezione di tipo UlegalStateException (erede di RuntimeException).
  - L’eccezione è generata anche a seguito di due chiamate consecutive
    di remove()
...
- Questi metodi si basano sull’indicizzazione degli elementi
  supportata da  =List=.
...
#+begin_src java
  ...
  add(T e);
#+end_src
- il metodo add di ListITerator aggiunge un elemento giusto prima del cursore
- next() non è influenzato dall'inserimento
- previous() ritorna l'elemento appena aggiunto

  #+begin_export  latex
\begin{figure}
\includegraphics[scale=0.26]{\figures/add.png}
\end{figure}
  #+end_export

** Discussione
- Il metodo set( x ) sostituisce x all'elemento corrente (definito   dall’ultima operazione next() o previous() eseguita).
- I metodi hasPrevious() e previous() sono duali di hasNext() e
  next(), e consentono di attraversare una lista a ritroso
#+begin_src  java
...
- Se il cursore si trova alla fine della lista, l'elemento viene
  aggiunto alla fine della lista
- Se il cursore è nella prima posizione, l'elemento viene aggiunto
  all'inizio della lista
- Se il cursore si trova nel mezzo della lista, l'elemento viene
  aggiunto prima del cursore
  - pertanto il cursore rimane in alterato

** Classi ArrayList<T> e LinkedList<T>
- Sono classi concrete che appoggiano la lista rispettavamente su un
  array nativo (che può espandersi e contrarsi) e su una lista concatenata
  - ArrayList è identico all'ArrayVector visto in precedenza
- Prima di decidere quale delle due classi adottare occorre riflettere
  su quale sia la più conveniente

  
** Lista concatenata
- Gli elementi adiacenti non sono contigui come nell'array, ma
  esplicitamente concatenati mediante riferimenti

#+begin_export latex
\begin{figure}
\includegraphics[scale=0.26]{\figures/linklist.png}
\end{figure}
#+end_export
...
- Aggiungendo l'elemento 6 tra il 2 e il 9
  #+begin_export latex
\begin{figure}
\includegraphics[scale=0.26]{\figures/linklist1.png}
\end{figure}
  #+end_export
**** Right                                                         :BMCOL:
:PROPERTIES:
...
- Rimuovendo l'elemento 9
  #+begin_export latex
\begin{figure}
\includegraphics[scale=0.26]{\figures/linklist2.png}
\end{figure}
  #+end_export
...
** Discussione
- Da notare che operazioni come getFirst() e getLast() di LinkedList
  sono ottenibili   anche tramite la funzione get definita
  nell'interfaccia List (resp. get(0), get(size()-1)).
- Utilizzare gli indici su una LinkedList (e.g., get(i)) è
  un'operazione inefficiente
